...
const SQL = {
  RECOMMENDATION: {
    ...
    GET_REVIEW_IMAGES : `SELECT * FROM pa_recommendation_service.product_reviews_media_storage WHERE review_id=$1 and blocked is null and deleted is null`,
    GET_REVIEW : `
	  SELECT prw.*, pr.rating FROM pa_recommendation_service.product_reviews prw 
  	  left join pa_recommendation_service.product_ratings pr on (pr.product_id = prw.product_id and pr.user_id = prw.user_id )  
  	WHERE 1=1 
    ...
    DELETE_REVIEW_IMAGE : `UPDATE pa_recommendation_service.product_reviews_media_storage set deleted=NOW() where media_id=$1 and user_id=$2 and deleted is null`, 
  },
};
